...
$(function () {
  ...
  var hl,
    newsList = $('.vs-headlines'),
    newsListItems = $('.vs-headlines li'),
    firstNewsItem = $('.vs-headlines li:nth-child(1)'),
    newsPreview = $('.vs-preview'),
    elCount = $('.vs-headlines').children(':not(.highlight)').length,
    vPadding = (parseInt(firstNewsItem.css('padding-top').replace('px', ''), 10)) +
      (parseInt(firstNewsItem.css('padding-bottom').replace('px', ''), 10)),
    vMargin = (parseInt(firstNewsItem.css('margin-top').replace('px', ''), 10)) +
      (parseInt(firstNewsItem.css('margin-bottom').replace('px', ''), 10)),
    cPadding = (parseInt($('.vs-content').css('padding-top').replace('px', ''), 10)) +
      (parseInt($('.vs-content').css('padding-bottom').replace('px', ''), 10)),
  ...
  function doEqualHeight(c) {
    ...
    if ($('.vs-content:nth-child(' + c + ')').height() > newsPreview.height()) {
      newsPreview.height($('.vs-content:nth-child(' + c + ')').height() + cPadding);
    }
  }
  ...
  $('.vs-content').on('mouseover', function () {
    clearInterval(myTimer);
  });

  // auto-switching starts again
  $('.vs-content').on('mouseout', function () {
    doTimedSwitch();
  });
  ...
  function doClickItem() {
    ...
    newsListItems.on('click', function () {
      ...
      $('.vs-content:nth-child(' + indexEl + ')').siblings().removeClass('top-content');
      $('.vs-content:nth-child(' + indexEl + ')').addClass('top-content');
